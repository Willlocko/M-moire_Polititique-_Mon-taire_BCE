...
# Cette section charge les données du fichier Excel, je l'ai nommé:Base_Panel1, elle contient mes observations (.xlsx).
...
# ==============================================================================
# Ici on Renomme les colonnes pour une meilleure lisibilité et pour correspondre aux variables de l'analyse.
...
# Ici on va Convertir la colonne 'Quarter' (ex: '2005Q1') en un index temporel DatetimeIndex.
# linearmodels nécessite un index temporel de type date.
try:
...
# On va Trier les données par pays puis par trimestre pour une structure de panel cohérente.
df = df.sort_values(by=['Country', 'Quarter_Datetime'])

# Définissons l'index de panel (MultiIndex) avec 'Country' comme identifiant de l'entité
...
print(df_panel.head())
# Affichons le type de l'index temporel pour confirmation
print(f"Type de l'index temporel : {df_panel.index.get_level_values(1).dtype}")


# On va Définir les variables dépendante (à expliquer) et indépendantes (explicatives).
dependent_var = df_panel['HICP']
exog_vars = df_panel[['GDP', 'Credit', 'FBCF', 'ECB_Rate', 'PEPP']]

# Ajoutons une constante (intercept) au modèle de régression, nécessaire pour l'estimation.
exog_vars = sm.add_constant(exog_vars)

#ici on va gérer les valeurs manquantes (NaN) : suppression des lignes complètes contenant des NaN.
...
# On va extraire les variables nettoyées pour l'estimation des modèles.
...
# résultats des modèles, utiles pour le mémoire.
# ==============================================================================
...
# Observons l'évolution de l'inflation au fil du temps pour des pays sélectionnés.
# ------------------------------------------------------------------------------
...
# Liste des pays inclus dans le graphique. 
...
# Visualisons la relation brute entre l'inflation et la croissance économique.
# ------------------------------------------------------------------------------
print("\n[Figure X.Y] : Relation entre l'HICP et la Croissance du PIB dans la Zone Euro (2005Q1-2023Q4)")
plt.figure()

# Réinitialisons l'index pour que 'Country' et 'Quarter_Datetime' deviennent des colonnes.
...
# Montrons que la tendanc agrégée de l'inflation et la période d'une politique clé.
# ------------------------------------------------------------------------------
...
# Ces graphiques vont aider à diagnostiquer l'adéquation du modèle.
# ------------------------------------------------------------------------------
...
# Ce code Fournit un aperçu de la distribution et de la variabilité des variables clés
...
# Définissons les trimestres futurs jusqu'à fin 2030.
#(NB: Au final je n'ai pas inclus cette analyse 2030 dans mon mémoire; il pourrait servir pour des études à venir)
...
# permet d'établir un DataFrame avec les valeurs des variables explicatives pour les périodes futures.
future_data_list = []
...
for country in countries_to_simulate_plot:
    ...
    #permet d'ajouter une ligne pour marquer le passage des données observées aux données prédites.
